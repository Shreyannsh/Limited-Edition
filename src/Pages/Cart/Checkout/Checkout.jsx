...
export default function Checkout() {
  ...
  const selectedAddress = addressList?.find(
    (address) => address._id === selectedAddress_Id
  );
  ...
  return (
    <div className="checkoutPage">
      ...
      <div className="orderDetails">
        ...
        {state.initialCart.map((order) => (
          <li className="itemName" key={order._id}>
            <p className="orderName">
              {" "}
              <span>{order.name}</span> <span>{order.qty} qty</span>
            </p>
          </li>
        ))}
        <h2>Price Details</h2>
        <p className="priceDetail">
          Cart Price <span>&#x20B9;{totalCartCount.totalAmount}</span>{" "}
        </p>
        <p className="priceDetail">
          Discount <span>&#x20B9;00</span>
        </p>
        <p className="priceDetail">
          Delivery Charges <span>&#x20B9;250</span>
        </p>
        <p className="priceDetail">
          Total Amount <span>&#x20B9;{totalCartCount.totalAmount + 250}</span>
        </p>
        <h2>DELIVER TO</h2>
        {selectedAddress ? (
